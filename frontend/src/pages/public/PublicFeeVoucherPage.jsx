...
export default function PublicFeeVoucherPage() {
  ...
  const handleContinue = async (e) => {
    ...
    setLoading(true);
    try {
      const res  = await api.get('/students', { params: { search: rollId.trim(), limit: 5 } });
      const list = res.data.data || [];
      const found = list.find(s => s.rollNo?.toLowerCase() === rollId.trim().toLowerCase());
      if (!found) return toast.error('No student found with this Roll ID.');
      const feeRes = await api.get('/fees/student/' + found.id);
      const schoolRes = await api.get('/settings/school').catch(() => ({ data: { data: null } }));
      setStudent(feeRes.data.data?.student || found);
      setInvoices(feeRes.data.data?.invoices || []);
      setSchool(schoolRes.data.data);
      if (feeRes.data.data?.invoices?.length > 0) {
        setSelected(feeRes.data.data.invoices[0]);
      }
      setStep(2);
    } catch {
