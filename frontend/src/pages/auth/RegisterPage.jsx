...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        localStorage.setItem('registeredSchoolEmail', form.email);
        localStorage.setItem('registeredSchoolName', form.schoolName);
        const slug = data.data?.school?.slug || form.schoolName.toLowerCase().replace(/[^a-z0-9]/g, '-');
  ...
  if (registered) {
    return (
      <div style={{ minHeight:'100vh', background:'#F0F4F8', display:'flex', alignItems:'center', justifyContent:'center', padding:20, fontFamily:"'Inter',system-ui,sans-serif" }}>
        <div style={{ width:'100%', maxWidth:520, background:'#fff', borderRadius:16, padding:'36px 40px', boxShadow:'0 8px 32px rgba(0,0,0,0.1)', textAlign:'center' }}>

          {/* Animated checkmark circle */}
          <div style={{ width:72, height:72, borderRadius:'50%', background:'linear-gradient(135deg,#0D9488,#0F766E)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 20px', boxShadow:'0 4px 20px rgba(13,148,136,0.35)' }}>
          ...
          <p style={{ color:'#64748B', fontSize:13.5, marginBottom:24, lineHeight:1.6 }}>
            Your school account has been created. Check your email for next steps.
          </p>

          {/* What's in the email */}
          <div style={{ background:'#F0FDF9', border:'1px solid #CCFBF1', borderRadius:12, padding:'18px 20px', marginBottom:22, textAlign:'left' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
              <Mail size={16} color="#0D9488"/>
              <span style={{ fontSize:13, fontWeight:700, color:'#0F766E' }}>Check your email at: {form.email}</span>
            </div>
            <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:9 }}>
              <li style={{ display:'flex', alignItems:'flex-start', gap:9, fontSize:13, color:'#1E3A5F' }}>
                <span style={{ fontSize:16, flexShrink:0 }}>1</span>
                <span><strong>A 6-digit OTP code</strong> to verify your account</span>
              </li>
              <li style={{ display:'flex', alignItems:'flex-start', gap:9, fontSize:13, color:'#1E3A5F' }}>
                <span style={{ fontSize:16, flexShrink:0 }}>2</span>
                <span><strong>Your auto-generated password</strong> (keep it safe!)</span>
              </li>
              <li style={{ display:'flex', alignItems:'flex-start', gap:9, fontSize:13, color:'#1E3A5F' }}>
                <span style={{ fontSize:16, flexShrink:0 }}>3</span>
                <span><strong>Your unique school login link</strong></span>
              </li>
            </ul>
          </div>

          <p style={{ fontSize:12.5, color:'#94A3B8', marginBottom:20, lineHeight:1.6 }}>
            Redirecting you to the verification page in a moment…
          </p>

          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
            <Loader2 size={15} color="#0D9488" style={{ animation:'spin 0.8s linear infinite' }}/>
            <span style={{ fontSize:12.5, color:'#0D9488', fontWeight:600 }}>Please wait…</span>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ width:'100%', maxWidth:560, background:'#fff', borderRadius:16, padding:'32px 36px', boxShadow:'0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ background:'#EFF6FF', border:'1px solid #BFDBFE', borderRadius:10, padding:'10px 12px', marginBottom:16 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#1D4ED8', textTransform:'uppercase', letterSpacing:'.04em', marginBottom:4 }}>
            Your Unique Branded School Login Link
          </div>
            ...
            <div className="form-group">
              ...
              <div style={{ position:'relative' }}>
                <input className="form-input" type="email" placeholder="admin@school.com"
                  value={form.email} onChange={e=>setForm({...form,email:e.target.value})} style={{ paddingLeft:32 }}/>
