...
/* ─── Bank Challan Print Helper ──────────────────── */
function printBankChallan(row, month, year) {
  const schoolName = localStorage.getItem('registeredSchoolName') || 'IlmForge School';
  const bankName   = localStorage.getItem('schoolBankName')       || 'Meezan Bank Limited';
  const accountNo  = localStorage.getItem('schoolBankAccount')    || 'PK00MEZN0001234567890';
  const branchCode = localStorage.getItem('schoolBranchCode')     || '0001';
  const date       = new Date().toLocaleDateString('en-PK');

  const html = `<!DOCTYPE html><html><head><title>Bank Challan — ${schoolName}</title>
  <style>
    body { font-family: Arial, sans-serif; font-size: 12px; margin: 0; padding: 0; }
    .page { max-width: 760px; margin: 0 auto; padding: 20px; }
    h2, h3 { text-align: center; margin: 3px 0; }
    .bank-info { border: 2px solid #1B2F6E; border-radius: 6px; padding: 12px 16px; margin: 14px 0; }
    .row { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px dashed #ccc; }
    .label { color: #555; font-weight: 600; } .val { font-weight: 700; }
    .copies { display: flex; gap: 20px; margin-top: 16px; }
    .copy { flex: 1; border: 1px solid #999; border-radius: 4px; padding: 10px; }
    .copy-title { font-size: 10px; font-weight: 700; color: #888; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid #eee; margin-bottom: 8px; padding-bottom: 4px; }
    .sig { margin-top: 20px; display: flex; justify-content: space-between; }
    .sig-line { border-top: 1px solid #333; width: 140px; text-align: center; font-size: 10px; color: #666; padding-top: 4px; }
    @media print { body { -webkit-print-color-adjust: exact; } .no-print { display: none; } }
  </style></head><body>
  <div class="page">
    <h2>${schoolName}</h2>
    <h3>Bank Deposit Slip / Fee Challan</h3>
    <p style="text-align:center;font-size:10px;color:#888;margin:2px 0">Date: ${date} &nbsp;|&nbsp; Month: ${month} ${year}</p>
    <div style="text-align:right;margin:8px 0" class="no-print">
      <button onclick="window.print()" style="background:#1B2F6E;color:#fff;border:none;padding:7px 18px;border-radius:6px;cursor:pointer;font-size:12px">Print Challan</button>
    </div>
    <div class="copies">
      ${['Bank Copy', 'School Copy', 'Student Copy'].map(copyTitle => `
        <div class="copy">
          <div class="copy-title">${copyTitle}</div>
          <div class="bank-info" style="font-size:11px">
            <div style="font-weight:700;color:#1B2F6E;margin-bottom:6px">${bankName}</div>
            <div class="row"><span class="label">Account No</span><span class="val" style="font-family:monospace">${accountNo}</span></div>
            <div class="row"><span class="label">Branch Code</span><span class="val">${branchCode}</span></div>
            <div class="row"><span class="label">Account Title</span><span class="val">${schoolName}</span></div>
          </div>
          <div class="row"><span class="label">Class</span><span class="val">${row?.className || '—'}</span></div>
          <div class="row"><span class="label">Section</span><span class="val">${row?.sectionName || '—'}</span></div>
          <div class="row"><span class="label">Month</span><span class="val">${month} ${year}</span></div>
          <div class="row"><span class="label">Total Challans</span><span class="val">${row?.challanCount || '—'}</span></div>
          <div class="row"><span class="label">Deposit Amount</span><span class="val">Rs. _______________</span></div>
          <div class="row"><span class="label">Student Name</span><span class="val">_____________________</span></div>
          <div class="row"><span class="label">Roll No</span><span class="val">___________</span></div>
          <div class="sig">
    ...
    <p style="text-align:center;font-size:9px;color:#aaa;margin-top:16px">Generated by IlmForge School Management System | ${date}</p>
  </div>
...
function FeeChallansTab() {
  ...
  return (
    <div>
      ...
      {fetched && !isLoading && (
        <DataTable
          headers={['S.No.', 'Class', 'Section', 'Download', 'Count', 'Delete', 'Bulk Action', 'Bank Challan']}
        ...
        >
          {mockRows.map((row, i) => (
            <tr key={`${row.classId}-${row.sectionId}`} style={{ background: i % 2 === 0 ? '#fff' : '#f9fafb', borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '10px 14px' }}>
                <button
                  onClick={() => printBankChallan(row, month, year)}
                  style={{ ...outlineBtn('#1B2F6E'), padding: '5px 10px' }}
                  title="Print Bank Deposit Slip"
                >
                  <Printer size={13} /> Bank Challan
                </button>
              </td>
            </tr>
          ))}
