...
export default function HolidayCalendarPage() {
  ...
  // Backed by the real /holidays API (HolidayEvent model) — previously this
  // page only wrote to localStorage, so a holiday declared by one admin was
  // invisible on any other device/session and to the attendance working-days
  // calculation entirely.
  const { data: rawHolidays = [], isLoading } = useQuery({
    queryKey: ['holidays'],
    queryFn: () => api.get('/holidays').then(r => r.data.data || []),
  });
  const holidays = rawHolidays.map(h => ({
    id: h.id, title: h.title, date: h.startDate, endDate: h.endDate, type: h.eventType, desc: h.notes,
  }));

  const addHoliday = useMutation({
    mutationFn: (data) => api.post('/holidays', {
