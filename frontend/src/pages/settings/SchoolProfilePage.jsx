...
import { Save, Upload, Camera, Palette, Building2, Globe, Phone, Mail, MapPin, CheckCircle } from 'lucide-react';
    ...
    <div className="page-content fade-up">
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
        <div>
          ...
          <p className="page-subtitle">Logo, colors and info used across the entire platform</p>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div className="card">
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
              <Camera size={15} color="#0D9488"/>
              <h3 style={{ margin:0, fontSize:14, fontWeight:700, color:'#1E3A5F' }}>School Logo</h3>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:20 }}>
              ...
              <div>
                <button type="button" onClick={()=>logoRef.current?.click()}
                  className="btn btn-teal btn-sm" style={{ marginBottom:8 }}>
                  <Upload size={13}/> {logoPreview?'Change Logo':'Upload Logo'}
                </button>
                <p style={{ fontSize:11.5,color:'#64748B',lineHeight:1.6 }}>
                  PNG or JPG, max 2MB.<br/>
                  Recommended: 250×250px with transparent background.<br/>
                  Appears on login page, sidebar, PDFs and ID cards.
                </p>
              </div>
          ...
          <div className="card">
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
              ...
              <h3 style={{ margin:0, fontSize:14, fontWeight:700, color:'#1E3A5F' }}>Brand Color</h3>
            </div>
            <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:14 }}>
              {THEME_PRESETS.map(t => (
                <div key={t.name} onClick={()=>{ setSelectedColor(t.p); setSelectedPreset(t.name); document.documentElement.style.setProperty('--teal',t.p); }}
                  style={{ display:'flex',alignItems:'center',gap:6,padding:'6px 11px',borderRadius:8,cursor:'pointer',border:`2px solid ${selectedPreset===t.name?t.p:'#E8EDF3'}`,background:selectedPreset===t.name?`${t.p}12`:'#FAFAFA',transition:'all 0.12s' }}>
                  <div style={{ width:14,height:14,borderRadius:'50%',background:t.p }}/>
                  <span style={{ fontSize:12,fontWeight:selectedPreset===t.name?600:400,color:selectedPreset===t.name?t.p:'#64748B' }}>{t.name}</span>
                  {selectedPreset===t.name&&<CheckCircle size={11} color={t.p}/>}
                </div>
              ))}
            </div>
            {/* Custom color */}
          ...
          <div className="card">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              {[
                ...
                { label:'City',            key:'city',    span:1, ph:'Islamabad'   },
                { label:'Phone',           key:'phone',   span:1, ph:'051-1234567' },
                { label:'Address',         key:'address', span:2, ph:'F-8 Markaz, Islamabad' },
                { label:'Email',           key:'email',   span:2, ph:'info@school.com', type:'email' },
              ].map(f => (
