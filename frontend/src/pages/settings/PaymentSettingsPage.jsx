...
const STORAGE_KEY = 'ilmforge_payment_settings';
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (data.bank)    setBankForm(prev => ({ ...prev, ...data.bank }));
        if (data.voucher) setVoucherForm(prev => ({ ...prev, ...data.voucher }));
      } catch {}
    }
  }, []);

  const handleSave = () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ bank: bankForm, voucher: voucherForm }));
    setSaved(true);
