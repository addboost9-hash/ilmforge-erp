...
/* Map backend rules array into a keyed object { [trigger]: rule } */
...
/* Build initial taskSettings from backend rules, falling back to TASK defaults */
function buildTaskSettings(rulesMap) {
  return TASKS.reduce((acc, task) => {
    const rule = rulesMap[task.id];
    if (rule) {
      let config = {};
      try { config = rule.config ? JSON.parse(rule.config) : {}; } catch {}
      acc[task.id] = { ...task.defaults, ...config, enabled: rule.isActive };
    } else {
      acc[task.id] = { ...task.defaults };
    }
    return acc;
  }, {});
}
...
export default function AutomationPage() {
  ...
  const { data: rules = [], isLoading } = useQuery({
    ...
    queryFn: () => api.get('/automation/rules').then(r => r.data.data),
  });

  /* Sync local state whenever backend data arrives */
  useEffect(() => {
    if (rules.length > 0) {
      const map = rulesArrayToMap(rules);
      setTaskSettings(buildTaskSettings(map));
    }
  }, [rules]);
  ...
  /* Save all tasks to backend (upsert per task) */
  const handleSave = async () => {
    setSaving(true);
    const rulesMap = rulesArrayToMap(rules);

    try {
      for (const task of TASKS) {
        const ts = taskSettings[task.id];
        const { enabled, ...configFields } = ts;
        const existingRule = rulesMap[task.id];
        ...
        const payload = {
          ...
          isActive: !!enabled,
          config:   configFields,
          runTime:  configFields.sendTime || configFields.backupTime || '09:00',
        };

        if (existingRule) {
          await api.put(`/automation/rules/${existingRule.id}`, payload);
        } else {
      ...
      toast.success(
        active.length
          ? `Automation saved! Active: ${active.join(', ')}`
          : 'Settings saved. All automations off.'
      );
  ...
  return (
    <div className="page-content fade-up">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <button className="btn btn-teal" onClick={handleSave} disabled={saving || isLoading}>
          {savedOk
      ...
      {isLoading && (
        <div style={{ textAlign: 'center', padding: 40, color: '#64748B' }}>Loading automation settings…</div>
      )}
