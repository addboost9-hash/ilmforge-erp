...
// Staff.basicSalary (and therefore incrementAmount, derived from it) is stored
// in paisa (rupee * 100) throughout this app — the same convention used by
// every other money field. The threshold must be compared in the same unit,
// or "Rs 50,000" silently becomes "Rs 500" (a 100x compliance-control bug).
const DUAL_APPROVAL_THRESHOLD_AMOUNT_RUPEES = 50000;
const DUAL_APPROVAL_THRESHOLD_AMOUNT = DUAL_APPROVAL_THRESHOLD_AMOUNT_RUPEES * 100;

const fmtDateTime = (d) => (d ? new Date(d).toLocaleString('en-PK') : '—');
const fmtRupees = (paisa) => Math.round(Number(paisa || 0) / 100).toLocaleString('en-PK');
...
router.post('/payroll/:adjustmentId/approve', wrap(async (req, res) => {
  ...
  if (!comment || !String(comment).trim()) {
    return res.status(400).json({ success: false, message: 'A signed approval comment is required.' });
  }
  ...
  if (dual && row.approvalStage === 0) {
    if (requiresSuperAdminPair && req.user?.role !== 'super_admin') {
      return res.status(403).json({
        success: false,
        message: `High-value adjustment requires super_admin first approval (>${DUAL_APPROVAL_THRESHOLD_PCT}% or >Rs ${DUAL_APPROVAL_THRESHOLD_AMOUNT_RUPEES.toLocaleString('en-PK')}).`,
      });
    }
  ...
  if (dual && row.approvalStage === 1) {
    ...
    if (requiresSuperAdminPair && req.user?.role !== 'super_admin') {
      return res.status(403).json({
        success: false,
        message: `High-value adjustment requires super_admin second approval (>${DUAL_APPROVAL_THRESHOLD_PCT}% or >Rs ${DUAL_APPROVAL_THRESHOLD_AMOUNT_RUPEES.toLocaleString('en-PK')}).`,
      });
    }
...
router.post('/payroll/:adjustmentId/apply', wrap(async (req, res) => {
  ...
  const { comment } = req.body || {};
  if (!comment || !String(comment).trim()) {
    return res.status(400).json({ success: false, message: 'A signed apply comment is required.' });
  }
  const row = await prisma.payrollAdjustment.findFirst({ where: { id: adjustmentId, schoolId } });
  if (!row) return res.status(404).json({ success: false, message: 'Adjustment not found.' });
...
router.post('/payroll/:adjustmentId/rollback', wrap(async (req, res) => {
  ...
  // Rollback reverses an already-applied salary change — this is the single
  // most sensitive payroll action, so a real, non-empty reason is mandatory
  // and enforced server-side (previously an empty reason silently became the
  // generic placeholder "Administrative rollback" instead of being rejected).
  if (!reason || !String(reason).trim()) {
    return res.status(400).json({ success: false, message: 'A rollback reason is required.' });
  }
  const row = await prisma.payrollAdjustment.findFirst({ where: { id: adjustmentId, schoolId } });
  if (!row) return res.status(404).json({ success: false, message: 'Adjustment not found.' });
