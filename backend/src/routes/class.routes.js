...
router.post('/subjects', wrap(async (req, res) => {
  const { classId, name, code, description, totalMarks, teacherId } = req.body;
  const subject = await prisma.subject.create({ data: { schoolId: req.schoolId, classId: parseInt(classId), name, code, description, totalMarks: totalMarks ? parseInt(totalMarks) : 100, teacherId: teacherId ? parseInt(teacherId) : null } });
  res.status(201).json({ success: true, data: subject });
}));

router.put('/subjects/:id', wrap(async (req, res) => {
  const existing = await prisma.subject.findFirst({ where: { id: parseInt(req.params.id), schoolId: req.schoolId } });
