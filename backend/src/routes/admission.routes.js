...
router.put('/inquiries/:id', wrap(async (req, res) => {
  const existing = await prisma.admissionInquiry.findFirst({ where: { id: parseInt(req.params.id), schoolId: req.schoolId } });
  if (!existing) return res.status(404).json({ success: false, message: 'Inquiry not found.' });
  const { status, notes } = req.body;
  const inquiry = await prisma.admissionInquiry.update({ where: { id: existing.id }, data: { status, notes, updatedAt: new Date() } });
