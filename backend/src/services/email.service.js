...
const sendEmail = async ({ to, subject, html, text }) => {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from:    `"${process.env.FROM_NAME || 'EduForge Pro'}" <${process.env.FROM_EMAIL || 'interface_alerts@carecloud.com'}>`,
      to,
      subject,
      html,
      text: text || subject,
...
const sendWelcomeEmail = async ({ to, name, schoolName, email, password, loginUrl, schoolSlug }) => {
  const platform    = 'IlmForge';
  const tagline     = 'Ilm Ko Asaan Banaye';
  // School-specific login link
  const schoolLogin = schoolSlug
    ? `https://${schoolSlug}.ilmforge.cloud/login`
    : `${loginUrl}/login`;
...
<body style="margin:0;padding:0;background:#F0FDFA;font-family:'Segoe UI','Helvetica Neue',Arial,sans-serif;">
  ...
  return sendEmail({
    to:      email,
    subject: `🎉 ${schoolName} is Live on IlmForge — Your Login Credentials`,
    html,
  });
};
...
<table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;background:#F0FDFA;">
  ...
  return sendEmail({
    to,
    subject: `🔐 Your ${schoolName} Verification Code: ${otp}`,
    html,
  });
};
...
const sendSchoolReadyEmail = async ({ name, email, password, schoolName, loginUrl, schoolSlug }) => {
  ...
  const finalUrl = loginUrl || (schoolSlug ? `https://${schoolSlug}.ilmforge.cloud/login` : '#');
...
<table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;background:#F0FDFA;">
      ...
      <tr><td style="background:#ffffff;padding:36px 40px 30px;">
        ...
        <div style="background:linear-gradient(135deg,#F0FDFA 0%,#ECFDF5 100%);border:2px solid #0D9488;border-radius:14px;padding:24px 28px;margin-bottom:26px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
            ...
            <!-- Password (only shown if explicitly provided — we no longer
                 persist plaintext passwords server-side, so this is omitted
                 on re-sends) -->
            <tr>
              ...
              <td style="padding:10px 0;">
                ${password
                  ...
                  : `<span style="color:#374151;font-size:12.5px;">Use the password sent in your registration email</span>`}
              </td>
            </tr>
  ...
  return sendEmail({
    to:      email,
    subject: `🎉 ${schoolName} is Ready on IlmForge — Your Login Credentials`,
    html,
  });
};

module.exports = { sendEmail, sendWelcomeEmail, sendOTPEmail, sendSchoolReadyEmail };
