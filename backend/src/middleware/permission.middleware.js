...
function resolveAction(req) {
  // Use the ORIGINAL request URL, not req.path — inside a mounted sub-router
  // (e.g. app.use('/api/v1/pdf', PM('reports'), pdfRouter)) Express strips the
  // mount prefix from req.path before this middleware runs, so a path like
  // '/api/v1/pdf/voucher/:id' becomes just '/voucher/:id' and the "pdf"
  // substring this check relies on silently disappears — letting a role with
  // canExport:false still download PDFs/vouchers/marksheets. originalUrl keeps
  // the full path regardless of mount depth.
